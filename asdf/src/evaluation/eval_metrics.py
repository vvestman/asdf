...
def _get_tar_and_nontar_scores(scores: np.ndarray, labels: List[str], tar_label: str, nontar_label: str) -> Tuple[np.ndarray, np.ndarray]:
    ...
    if not tar_label:
        tar_labels = {'Target', 'target', 'TARGET', '1', 't', 'T'}
    else:
        tar_labels = {tar_label}

    if not nontar_label:
        nontar_labels = {'Nontarget', 'nontarget', 'NONTARGET', 'Non-target', 'non-target', 'NON-TARGET', '0', 'n', 'N',
                         'f', 'F'}
    else:
        nontar_labels = {nontar_label}
...
def compute_eer_from_score_file(score_file):
    cm_data = np.genfromtxt(score_file, dtype=str)
    cm_keys = cm_data[:, 1].astype(bool)
    cm_scores = cm_data[:, 2].astype(float)
    bona_scores = cm_scores[cm_keys]
    spoof_scores = cm_scores[~cm_keys]
    return _compute_eer(bona_scores, spoof_scores)[0] * 100
