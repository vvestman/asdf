...
class StreamingEpochBatchIterator(EpochBatchIterating):
    """A steaming-style iterator over a :class:`torch.utils.audio_utils.IterableDataset`.

    Args:
        dataset (~torch.utils.audio_utils.Dataset): dataset from which to load the audio_utils
        max_sentences: batch size
        collate_fn (callable): merges a list of samples to form a mini-batch
        num_workers (int, optional): how many subprocesses to use for audio_utils
            loading. 0 means the audio_utils will be loaded in the main process
            (default: 0).
        epoch (int, optional): the epoch to start the iterator from
            (default: 1).
        buffer_size (int, optional): the number of batches to keep ready in the
            queue. Helps speeding up dataloading. When buffer_size is zero, the
            default torch.utils.audio_utils.DataLoader preloading is used.
...
class EpochBatchIterator(EpochBatchIterating):
    """A multi-epoch iterator over a :class:`torch.utils.audio_utils.Dataset`.

    Compared to :class:`torch.utils.audio_utils.DataLoader`, this iterator:
    ...
    Args:
        dataset (~torch.utils.audio_utils.Dataset): dataset from which to load the audio_utils
        collate_fn (callable): merges a list of samples to form a mini-batch
        batch_sampler (~torch.utils.audio_utils.Sampler or a callable): an iterator over batches of
            indices, or a callable to create such an iterator (~torch.utils.audio_utils.Sampler).
            A callable batch_sampler will be called for each epoch to enable per epoch dynamic
            batch iterators defined by this callable batch_sampler.
        seed (int, optional): seed for random number generator for
            reproducibility (default: 1).
        num_shards (int, optional): shard the audio_utils iterator into N
            shards (default: 1).
        shard_id (int, optional): which shard of the audio_utils iterator to
            return (default: 0).
        num_workers (int, optional): how many subprocesses to use for audio_utils
            loading. 0 means the audio_utils will be loaded in the main process
            (default: 0).
...
class BufferedIterator(object):
    ...
    def __next__(self):
        ...
        # Notify the user if there is a audio_utils loading bottleneck
            ...
            if time.time() - self.start_time > 5 * 60:
                if (
                    ...
                    logger.debug(
                        ...
                        "indicate a audio_utils loading bottleneck, and increasing the "
...
class GroupedEpochBatchIterator(EpochBatchIterator):
    ...
    1) audio_utils from one audio_utils set each time
    2) for different workers, they use the same order to fetch the audio_utils
    so they will use audio_utils from the same dataset everytime
