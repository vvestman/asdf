...
class HuffmanMMapIndex:
    ...
    def __init__(self, path):
        with open(path, "rb") as stream:
            ...
            # read length of audio_utils file
            (self._data_len,) = struct.unpack("<Q", stream.read(8))
            # read number of items in audio_utils file/index
...
class HuffmanMMapIndexedDataset(torch.utils.data.Dataset):
    """
    an indexed dataset that use mmap and memoryview to access audio_utils from disk
