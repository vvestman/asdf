...
class MultilingualDatasetManager(object):
    ...
    def add_args(parser):
        parser.add_argument(
            "audio_utils",
            help="colon separated path to audio_utils directories list, \
        ...
        parser.add_argument(
            "--extra-audio_utils",
            help='a dictionary of audio_utils name to this path, \
        ...
        parser.add_argument(
            "--extra-lang-pairs",
            help='a dictionary of audio_utils name to the language pairs they serve, \
        ...
        parser.add_argument(
            "--langtoks-specs",
            help='a list of comma separated audio_utils types that a set of language tokens to be specialized for, \
                            e.g. "main,dae,mined". There will be a set of language tokens added to the vocab to \
                            distinguish languages in different training audio_utils types. If not specified, default language \
        ...
        parser.add_argument(
            ...
            help='a file contain a python dictionary of how to sample audio_utils sets, \
        ...
        parser.add_argument(
            "--sampling-weights",
            help='a dictionary of how to sample audio_utils sets, \
                            e.g. { "main:en_XX-es_XX": 0.2, "mined:en_XX-pt_XX": 0.5, \
                                   "mono_dae:es_XX-es_XX: 0.3, "main:en_xx-fr_XX": 0.8 }',
            default=None,
            type=lambda uf: eval_str_dict(uf, type=str),
        ...
        parser.add_argument(
            ...
            default=None,
            type=int,
            help="virtual epoch size to speed up audio_utils loading",
        )
        parser.add_argument(
            "--virtual-audio_utils-size",
            default=None,
            type=int,
            help="virtual audio_utils size of the whole joint dataset to speed"
            "up audio_utils loading and have specific dynamic sampling strategy interval",
        )
    ...
    def estimate_global_pass_epoch(self, epoch):
        if self.args.virtual_epoch_size is None or self.args.virtual_data_size is None:
            return None
        # one epoch more for remaining audio_utils in each shard
    ...
    def load_langpair_dataset(
        ...
        if (
            ...
            # reversed directions of valid and test audio_utils will not share source and target datasets
            ...
            if langpairs_sharing_datasets is not None:
                ...
                if align_dataset is None:
                    # no align audio_utils so flag the reverse direction as well in sharing
                    langpairs_sharing_datasets[
    ...
    def load_a_dataset(
        ...
        **extra_kwargs,
        ...
        # TODO: handle modified lang toks for mined audio_utils and dae audio_utils
        if self.args.lang_tok_replacing_bos_eos:
    ...
    def get_data_paths_and_lang_pairs(self, split):
        ...
        if split == getattr(self.args, "train_subset", None):
            # only training audio_utils can have extra audio_utils and extra language pairs
        ...
        for data_category, paths in data_paths.items():
            ...
            for src, tgt in lang_dirs:
                ...
                if "mono_" in data_category:
                    # monolingual audio_utils requires tgt only
                    ...
                    if f"{src}-{tgt}" in shards_dict:
                        ...
                        # follow the fairseq tradition to use reversed direction audio_utils if it is not available
    ...
    def get_split_data_param_list(self, split, epoch, shard_epoch=None):
        # TODO: to extend with extra datasets and keys and loop over different shard audio_utils paths
        param_list = []
        data_paths, lang_pairs = self.get_data_paths_and_lang_pairs(split)
        ...
        for data_category, paths in data_paths.items():
            ...
            if split != getattr(self.args, "train_subset", None):
                # if not training audio_utils set, use the first shard for valid and test
                paths = paths[:1]
    ...
    def get_train_dataset_sizes(
        ...
        for (key, d), num_shard in zip(datasets, num_shards):
            ...
            data_sizes.append(
                # If we don't know the audio_utils size of the shard yet,
                # use the the max known audio_utils size to approximate.
                # Note that we preprocess shards by a designated shard size
                # and put any remaining audio_utils at the end into the last shard so
                ...
                # exact audio_utils sizes of the whole audio_utils size.
                (key, sum(my_data_sizes.get(i, known_size) for i in range(num_shard)))
            )
        logger.info(
            f"estimated total audio_utils sizes of all shards used in sampling ratios: {data_sizes}. "
            "Note that if the audio_utils a shard has not been loaded yet, use the max known audio_utils size to approximate"
        )
