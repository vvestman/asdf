...
class SampledMultiDataset(FairseqDataset):
    ...
    Args:
        datasets (
            List[~torch.utils.audio_utils.Dataset]
            or OrderedDict[str, ~torch.utils.audio_utils.Dataset]
        ): datasets
        ...
        collate_format (CollateFormat):  collater output format, either CollateFormat.ordered_dict or
            ...
            the collater to output batches of audio_utils mixed from all sub-datasets,
            and CollateFormat.ordered_dict configures the collater to output a dictionary of batches indexed by keys
            of sub-datasets.
            Note that not all sub-datasets will present in a single batch in both formats.
        virtual_size (int, or callable): the expected virtual size of the dataset (default: default_virtual_size_func).
        split (str): the split of the audio_utils, e.g. 'train', 'valid' or 'test'.
        shared_collater (bool): whether or not to all sub-datasets have the same collater.
        shuffle (bool): whether or not to shuffle audio_utils (default: True).
    """
    ...
    def __init__(
        ...
        # self.datasets[k][self._cur_indices[i]] is the audio_utils item i in this sampled dataset
        # namely, audio_utils item i is sampled from the kth sub-dataset self.datasets[k]
    ...
    def get_virtual_indices(self, rng, datasets, sample_ratios, virtual_size):
        ...
        if virtual_size < sum(sizes):
            logger.warning(
                f"virtual audio_utils size ({virtual_size}) is less than real audio_utils size ({sum(sizes)})."
                " If virtual size << real audio_utils size, there could be audio_utils coverage issue."
            )
