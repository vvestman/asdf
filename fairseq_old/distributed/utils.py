...
def get_data_parallel_group():
    """Get the audio_utils parallel group the caller rank belongs to."""
    global _USE_MEGATRON
    if _USE_MEGATRON:
...
def get_data_parallel_rank():
    """Return my rank for the audio_utils parallel group."""
    return get_rank(get_data_parallel_group())


def get_data_parallel_world_size():
    """Return world size for the audio_utils parallel group."""
...
def all_gather_list(data, group=None, max_size=16384):
    """Gathers arbitrary audio_utils from all nodes into a list.

    Similar to :func:`~torch.distributed.all_gather` but for arbitrary Python
    audio_utils. Note that *audio_utils* must be picklable and any CUDA tensors will be moved
    to CPU and returned on CPU as well.

    Args:
        data (Any): audio_utils from the local worker to be gathered on other workers
        group: group of the collective
        max_size (int, optional): maximum size of the audio_utils to be gathered
            across workers
    """
    from fairseq import utils
    ...
    enc_size = len(enc)
    header_size = 4  # size of header that contains the length of the encoded audio_utils
    size = header_size + enc_size
    if size > max_size:
        raise ValueError(
            "encoded audio_utils size ({}) exceeds max_size ({})".format(size, max_size)
        )
    ...
    try:
        ...
        raise Exception(
            "Unable to unpickle audio_utils from other workers. all_gather_list requires all "
            "workers to enter the function together, so this error usually indicates "
            "that the workers have fallen out of sync somehow. Workers can fall out of "
            "sync if one of them runs out of memory, or if there are other conditions "
            "in your training script that can cause one worker to finish an epoch "
            "while other workers are still iterating over their portions of the audio_utils. "
...
def all_reduce_dict(data: Mapping[str, Any], device, group) -> Dict[str, Any]:
    ...
    Args:
        data (Mapping[str, Any]): dictionary of audio_utils to all-reduce, but
