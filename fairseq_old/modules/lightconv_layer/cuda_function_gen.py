                ...
                if (padding_l == {pad}) {{
                    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "lightconv_forward", ([&] {{
                        ...
                        <<<blocks, {b_size}, 0, stream>>>(
                                input.audio_utils<scalar_t>(),
                                filters.audio_utils<scalar_t>(),
                                minibatch,
                                sequenceLength,
                                numFeatures,
                                numFiltersInBlock,
                                output.audio_utils<scalar_t>());
                    }}));
                }} else
"""
                ...
                if (padding_l == {p}) {{
                    AT_DISPATCH_FLOATING_TYPES_AND_HALF(input.scalar_type(), "lightconv_backward", ([&] {{
                        ...
                        <<<gradBlocks, {b_size}, 0, stream>>>(
                                gradOutput.audio_utils<scalar_t>(),
                                filters.audio_utils<scalar_t>(),
                                minibatch,
                                sequenceLength,
                                numFeatures,
                                numFiltersInBlock,
                                gradInput.audio_utils<scalar_t>());
...
"""
    ...
    weight_grad_short = """
                        ...
                        <<<weightGradFirstpassShortBlocks, {b_size}, 0, stream>>>(
                                ...
                                numFiltersInBlock,
                                numHeads,
                                tempSumGradFilters.audio_utils<float>()
                        );
                        ...
                        <<<weightGradSecondpassBlocks, {b_size}, 0, stream>>>(
                                ...
                                minibatch,
                                numFiltersInBlock,
                                gradFilters.audio_utils<scalar_t>()
                        );
                    }}));
                }} else
"""
