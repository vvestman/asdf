...
class TranslationFromPretrainedXLMTask(TranslationTask):
    ...
    we can load audio_utils that was binarized with the MaskedLMDictionary class.

    This task should be used for the entire training pipeline when we want to
    train an NMT model from a pretrained XLM checkpoint: binarizing NMT audio_utils,
