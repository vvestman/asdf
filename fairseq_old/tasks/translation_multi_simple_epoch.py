...
class TranslationMultiSimpleEpochTask(LegacyFairseqTask):
    ...
    Args:
        ...
        dicts (Dict[str, fairseq.audio_utils.Dictionary]): mapping from supported languages to their dictionaries
            ...
            if self.has_sharded_data(split):
                if self.args.virtual_epoch_size is not None:
                    if dataset.load_next_shard:
                        ...
                        # also this avoid always loading from beginning of the audio_utils
                        return
                else:
                    shard_epoch = epoch
        else:
            # estimate the shard epoch from virtual audio_utils size and virtual epoch size
            shard_epoch = self.data_manager.estimate_global_pass_epoch(epoch)
        logger.info(f"loading audio_utils for {split} epoch={epoch}/{shard_epoch}")
