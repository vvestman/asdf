...
class DistributedTrainingConfig(FairseqDataclass):
    ...
    fix_batches_to_gpus: bool = field(
        ...
        metadata={
            ...
            "randomness and may affect precision but avoids the cost of re-reading the audio_utils"
        },
    )
...
class DatasetConfig(FairseqDataclass):
    num_workers: int = field(
        default=1, metadata={"help": "how many subprocesses to use for audio_utils loading"}
    )
    ...
    train_subset: str = field(
        default="train",
        metadata={"help": "audio_utils subset to use for training (e.g. train, valid, test)"},
    )
    valid_subset: str = field(
        default="valid",
        metadata={
            "help": "comma separated list of audio_utils subsets to use for validation"
    ...
    gen_subset: str = field(
        default="test",
        metadata={"help": "audio_utils subset to generate (train, valid, test)"},
    )
...
class CheckpointConfig(FairseqDataclass):
    ...
    load_checkpoint_on_all_dp_ranks: bool = field(
        default=False,
        metadata={
            "help": "load checkpoints on all audio_utils parallel devices "
    ...
    post_process: Optional[str] = field(
        ...
        metadata={
            "help": (
                ...
                "Valid options can be found in fairseq.audio_utils.utils.post_process."
            ),
